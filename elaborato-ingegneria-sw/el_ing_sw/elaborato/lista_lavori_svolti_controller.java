...
public class lista_lavori_svolti_controller implements Initializable {
	...
	public Stage showEditLavoroSvolto(Lavoro_svolto lavoro_svolto) {
		...
		try {
			...
			edit_lavoro_svolto_controller controller = loader.getController();
	...
	public Stage showEditLavoroSvolto() {
		return this.showEditLavoroSvolto(new Lavoro_svolto(0, 0, LocalDate.now(), LocalDate.now(), "", "", "", new BigDecimal(0)));
	}
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
		id_lavoratore.setCellValueFactory(
				ls -> {
					try {
						return new SimpleStringProperty(anagLavDAO.getAnagrafiche_Lavoratori(ls.getValue().getId_lavoratore()).toString());
